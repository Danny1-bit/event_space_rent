...
const ClientSpaceList = ({ activeTab, setActiveTab }) => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const { data } = await axios.get("http://localhost:3030/api/v1/client/get-space-clients");
        setClients(data);
      } catch (error) {
        console.error("Error fetching space clients:", error);
        toast.error("Error loading space clients.");
      }
    };
    fetchClients();
  }, []);
